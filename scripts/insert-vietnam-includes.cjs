...
async function insertIncludes() {
  ...
  await pool.query(
    `INSERT INTO activities (id, trip_id, type, date, time, price, details)
     VALUES ('act-vietnam-incluye', 'vietnam-2026', 'conditions', '2026-06-19', '18:00', 0, $1)
     ON CONFLICT (id) DO UPDATE 
     SET trip_id = 'vietnam-2026',
         type = 'conditions',
         date = '2026-06-19',
         time = '18:00',
         price = 0,
  ...
  console.log('✅ Successfully inserted act-vietnam-incluye into activities table');
  await pool.end();
}

insertIncludes().catch(err => {
  console.error(err);
  process.exit(1);
});
