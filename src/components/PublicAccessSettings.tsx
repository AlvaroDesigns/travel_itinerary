    ...
    <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-[#009688]"><Link2 className="h-5 w-5" /></div>
          <div>
      ...
      {settings.publicAccessEnabled && <div className="mt-5 rounded-xl border border-teal-200/80 bg-teal-50/60 p-4">
        {publicUrl ? <><label className="text-xs font-bold uppercase tracking-wider text-[#00796b]">Enlace para compartir</label><div className="mt-2 flex gap-2"><input readOnly value={publicUrl} className="min-w-0 flex-1 rounded-lg border border-teal-200 bg-white px-3 py-2 text-xs text-zinc-700 outline-none" /><button type="button" onClick={copyUrl} className="flex shrink-0 items-center gap-1 rounded-lg bg-[#009688] px-3 py-2 text-xs font-bold text-white hover:bg-[#00796b]"><Copy className="h-3.5 w-3.5" />{copied ? 'Copiado' : 'Copiar'}</button><a href={publicUrl} target="_blank" rel="noreferrer" className="flex shrink-0 items-center justify-center rounded-lg border border-teal-200 bg-white px-3 text-[#00796b] hover:bg-teal-100" title="Abrir enlace público"><ExternalLink className="h-4 w-4" /></a></div></> : <p className="text-sm text-[#00796b]">Guarda la configuración para generar la URL pública.</p>}
      </div>}
