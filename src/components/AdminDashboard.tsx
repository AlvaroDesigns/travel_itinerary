...
export function AdminDashboard() {
  ...
  if (!data) return <div className="flex min-h-64 items-center justify-center gap-2 text-sm font-medium text-zinc-500"><Loader2 className="h-5 w-5 animate-spin text-[#0066FF]" /> Cargando analítica…</div>;

  const { overview } = data;
  const cards = [
    { label: 'Usuarios activos', value: overview.activeUsers, detail: `${overview.totalUsers} registrados · ${overview.adminUsers} administradores`, icon: Users, tone: 'bg-blue-50 text-[#0066FF]' },
  ...
  return <div className="w-full space-y-6">
    <div className="mb-7 flex flex-wrap items-end justify-between gap-5">
      <div>
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-blue-200/80 bg-blue-50 px-3.5 py-1 text-xs font-bold text-[#0066FF]"><Sparkles className="h-3.5 w-3.5 text-[#0066FF]" /> Centro de control</div>
    ...
    <section className="relative mb-7 overflow-hidden rounded-3xl bg-gradient-to-r from-blue-950 via-[#003399] to-blue-900 p-6 text-white shadow-xl sm:p-8">
      <div className="pointer-events-none absolute -right-10 -top-10 h-48 w-48 rounded-full bg-[#0066FF]/20 blur-3xl" />
      <div className="relative z-10 flex flex-col justify-between gap-5 md:flex-row md:items-center"><div><p className="text-xs font-bold uppercase tracking-wider text-blue-300">Próximo paso</p><h2 className="mt-1 text-2xl font-extrabold tracking-tight">Mantén actualizada la página de inicio</h2><p className="mt-2 max-w-xl text-sm leading-relaxed text-blue-100">Revisa el mensaje principal que ven los viajeros y publícalo en un solo paso.</p></div><Link href="/admin/paginas/inicio" className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-full bg-white px-6 text-sm font-bold text-[#0066FF] shadow-md transition-all hover:bg-blue-50 hover:scale-105 active:scale-95"><FileText className="h-4 w-4" /> Abrir editor <ArrowRight className="h-4 w-4" /></Link></div>
    </section>
    ...
    <div className="grid gap-6 xl:grid-cols-5">
      <section className="overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-xs xl:col-span-3">
        ...
        {data.activityBreakdown.length === 0 ? <p className="p-10 text-center text-sm text-zinc-500">Todavía no hay actividades registradas.</p> : <div className="divide-y divide-zinc-100">{data.activityBreakdown.map((item) => <div key={item.type} className="flex items-center justify-between gap-4 px-5 py-4 sm:px-6"><div><p className="font-bold text-zinc-800">{activityLabels[item.type] ?? item.type}</p><p className="mt-1 text-xs font-medium text-zinc-500">{item.count} {item.count === 1 ? 'actividad' : 'actividades'}</p></div><p className="rounded-lg bg-blue-50 px-2.5 py-1.5 text-sm font-bold text-[#0066FF]">{formatCurrency(item.spend)}</p></div>)}</div>}
      </section>
      <section className="overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-xs xl:col-span-2">
        <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-5 sm:px-6"><h2 className="font-bold text-zinc-950">Nuevos usuarios</h2><Link href="/usuarios" className="text-xs font-bold text-[#0066FF] hover:text-[#0052CC]">Ver todos</Link></div>
        {data.recentUsers.length === 0 ? <p className="p-10 text-center text-sm text-zinc-500">No hay usuarios todavía.</p> : <div className="divide-y divide-zinc-100">{data.recentUsers.map((user) => <div key={user.id} className="flex items-center gap-3 px-5 py-4 sm:px-6"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[10px] font-extrabold text-[#0066FF]">{user.email.slice(0, 2).toUpperCase()}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-zinc-800">{user.email}</p><p className="mt-1 flex items-center gap-1 text-xs font-medium text-zinc-500"><CalendarDays className="h-3 w-3" /> {formatDate(user.createdAt)}</p></div><span className="rounded-md bg-zinc-100 px-2 py-1 text-[10px] font-bold uppercase text-zinc-500">{user.role}</span></div>)}</div>}
      </section>
    </div>
    <section className="mt-6 flex items-center gap-4 rounded-2xl border border-blue-200/80 bg-blue-50/70 p-5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-[#0066FF] shadow-sm"><Users className="h-5 w-5" /></span><p className="text-sm leading-relaxed text-[#003399]"><strong>{overview.newUsersLast30Days}</strong> {overview.newUsersLast30Days === 1 ? 'usuario nuevo se ha unido en los últimos 30 días.' : 'usuarios nuevos se han unido en los últimos 30 días.'}</p></section>
  </div>;
}
