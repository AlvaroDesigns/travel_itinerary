...
export function HomeContentEditor() {
  ...
  if (isLoading) return <div className="flex min-h-64 items-center justify-center gap-2 text-sm font-medium text-zinc-500"><Loader2 className="h-5 w-5 animate-spin text-[#0066FF]" /> Cargando página…</div>;

  const inputClass = 'mt-2 w-full rounded-2xl border border-zinc-200 bg-zinc-50/60 px-4 text-sm text-zinc-800 outline-none transition focus:border-[#0066FF] focus:bg-white focus:ring-4 focus:ring-[#0066FF]/15';
  return <div className="mx-auto max-w-5xl">
    <div className="mb-7 flex flex-wrap items-end justify-between gap-5"><div><Link href="/admin/paginas" className="mb-4 inline-flex items-center gap-1.5 text-xs font-bold text-[#0066FF] hover:text-[#0052CC]"><ArrowLeft className="h-3.5 w-3.5" /> Volver a páginas</Link><h1 className="text-3xl font-extrabold tracking-tight text-zinc-950 sm:text-4xl">Editar inicio</h1><p className="mt-2 text-sm text-zinc-500 sm:text-base">Actualiza el mensaje principal de tu web.</p></div><Link href="/" className="inline-flex h-11 items-center gap-2 rounded-xl border border-zinc-200 bg-white px-4 text-sm font-bold text-zinc-700 shadow-xs transition hover:border-[#0066FF] hover:text-[#0052CC]"><Eye className="h-4 w-4" /> Ver página</Link></div>
