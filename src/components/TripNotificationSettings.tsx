...
const inputClassName = 'mt-1.5 h-10 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-700 outline-none transition focus:border-[#0066FF] focus:ring-2 focus:ring-[#0066FF]/15 disabled:cursor-not-allowed disabled:opacity-60';
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-xs">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[#0066FF]"><Bell className="h-5 w-5" /></div>
            <div>
      ...
      <div className={`rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-xs ${dependentClassName}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[#0066FF]"><CalendarClock className="h-5 w-5" /></div>
            <div>
        ...
        <div className="mt-5 flex flex-col sm:flex-row sm:items-end justify-between gap-5">
          <div className="flex flex-wrap items-end gap-4 sm:gap-5">
            <div className="w-full sm:w-56">
              <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500" htmlFor="reminder-frequency">
                Frecuencia
              </label>
              <select
                ...
                value={
                  [1, 2, 3, 7, 14, 30].includes(settings.reminderIntervalDays)
                    ? String(settings.reminderIntervalDays)
                    : 'custom'
                }
                onChange={(event) => {
                  ...
                  if (val === 'custom') {
                    ...
                    updateSettings({ reminderIntervalDays: Number(val) });
                  }
                }}
              >
                <option value="1">Diario (cada día)</option>
                <option value="2">Día sí, día no (cada 2 días)</option>
                <option value="3">Cada 3 días</option>
                <option value="7">Semanal (1 vez a la semana)</option>
                <option value="14">Quincenal (cada 2 semanas)</option>
                <option value="30">Mensual (cada 30 días)</option>
                {![1, 2, 3, 7, 14, 30].includes(settings.reminderIntervalDays) && (
                  <option value="custom">Personalizado ({settings.reminderIntervalDays} días)</option>
                )}
              </select>
            </div>

            {![1, 2, 3, 7, 14, 30].includes(settings.reminderIntervalDays) && (
              <div className="w-full sm:w-28 animate-in fade-in duration-200">
      ...
      <div className={`rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-xs ${dependentClassName}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[#0066FF]"><Mail className="h-5 w-5" /></div>
            <div>
      ...
      <div className="flex justify-end">
        <button type="button" disabled={isSaving} onClick={saveSettings} className="wanderlust-primary-button inline-flex h-12 items-center gap-2 rounded-full px-8 text-sm font-bold text-white shadow-md disabled:cursor-wait disabled:opacity-70">
