...
function readInvitationFromFragment(): Invitation | null {
  ...
  if (fragment.get('welcome') !== '1' || !challengeId || !/^[0-9a-f-]{36}$/i.test(challengeId) || !code || !/^\d{6}$/.test(code)) return null;
...
export default function Login() {
  ...
  const [notice, setNotice] = useState<string | null>(invitation ? 'Bienvenido. Elige una contraseña para activar tu cuenta.' : null);
  ...
  const requestCode = async (event?: React.FormEvent) => {
    ...
    try {
      ...
      setView('verify');
      setNotice('Si existe una cuenta activa, recibirás un código de seis dígitos en tu correo.');
    } catch (err) {
  ...
  const resetPassword = async (event: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('/api/auth/password-reset/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ challengeId, code: otp, password: newPassword }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'No se pudo restablecer la contraseña');
      backToLogin();
      setNotice(isInvitation ? 'Cuenta activada. Ya puedes iniciar sesión.' : 'Contraseña restablecida. Ya puedes iniciar sesión.');
    } catch (err) {
  ...
  return <div className="relative flex min-h-screen w-full items-center justify-center bg-ink-900 px-4 font-sans"><div className="relative z-10 w-full max-w-md"><div className="mb-10 flex justify-center"><Image src="/wanderlust_horizontal_blanco.png" alt="Wanderlust" width={260} height={70} priority className="h-16 w-auto object-contain" /></div><div className="rounded-2xl border border-white/10 bg-ink-800/60 p-8"><div className="mb-8 text-center"><h1 className="text-lg font-bold tracking-tight text-white">{view === 'login' ? 'Bienvenido de nuevo' : view === 'request' ? 'Recupera tu acceso' : isInvitation ? 'Crea tu contraseña' : 'Introduce el código'}</h1><p className="mt-1 text-xs font-medium uppercase tracking-[0.2em] text-ink-400">Planificador de viajes</p></div>{message}
    ...
    {view === 'verify' && <form onSubmit={resetPassword} className="space-y-5"><p className="text-center text-sm leading-relaxed text-ink-300">{isInvitation ? 'Elige una contraseña segura para activar tu cuenta.' : 'Introduce el código recibido y elige una nueva contraseña.'}</p>{!isInvitation && <div className="flex justify-center"><InputOTP value={otp} onChange={setOtp} maxLength={6} pattern={REGEXP_ONLY_DIGITS} inputMode="numeric" autoComplete="one-time-code" isDisabled={isLoading} inputClassName="text-white"><InputOTP.Group>{[0,1,2,3,4,5].map((index) => <InputOTP.Slot key={index} index={index} className="h-11 w-10 border border-white/20 bg-ink-900 text-lg font-bold text-white sm:w-11" />)}</InputOTP.Group></InputOTP></div>}<label className="block space-y-1.5"><span className="ml-1 block text-[11px] font-bold uppercase tracking-widest text-ink-400">Nueva contraseña</span><span className="relative block"><KeyRound className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-400" /><input type="password" minLength={12} required className={`${inputClass} pl-11`} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} /></span></label><label className="block space-y-1.5"><span className="ml-1 block text-[11px] font-bold uppercase tracking-widest text-ink-400">Repite la contraseña</span><input type="password" minLength={12} required className={inputClass} value={confirmation} onChange={(e) => setConfirmation(e.target.value)} /></label><button type="submit" disabled={isLoading || otp.length !== 6} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-white text-sm font-bold text-ink-900 disabled:opacity-50">{isLoading && <Loader2 className="h-4 w-4 animate-spin" />}{isInvitation ? 'Activar mi cuenta' : 'Guardar nueva contraseña'}</button>{!isInvitation && <button type="button" disabled={isLoading} onClick={() => void requestCode()} className="block w-full text-xs font-semibold text-ink-300 hover:text-white">Reenviar código</button>}</form>}
