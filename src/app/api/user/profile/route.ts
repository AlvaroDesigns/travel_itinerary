  ...
  try {
    ...
    const userRes = await pool.query(
      `SELECT u.id, u.email, u.role, u.name, u.phone, u.company, u.tenant_id, u.agency_name, u.preferences, u.created_at,
              ...
              ts.agency_cif AS tenant_cif, ts.terms_text AS tenant_terms
       FROM users u
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (session.tenantId && session.tenantId !== 'particular' && preferences) {
      ...
      await pool.query(
        `INSERT INTO tenant_settings (tenant_id, agency_name, agency_logo, brand_color, agency_cif, terms_text, updated_at)
         VALUES ($1, $2, COALESCE($3, ''), COALESCE($4, '#0066FF'), COALESCE($5, ''), COALESCE($6, ''), CURRENT_TIMESTAMP)
        ...
        [session.tenantId, agencyName, tenantLogo, tenantColor, tenantCif, tenantTerms]
      );

      // Keep all users of this tenant in sync with the agency logo
      if (tenantLogo !== null) {
        await pool.query(
          `UPDATE users
